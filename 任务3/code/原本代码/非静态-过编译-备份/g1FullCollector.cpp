...
uint G1FullCollector::calc_active_workers() {
  ...
  uint max_wasted_regions_allowed = ((heap->num_regions() * cur_HWP) / 100);
...
G1AdaptiveParameterControl G1FullCollector::adaptive_mixedgc(G1MixedGCLiveThresholdPercent, G1HeapWastePercent, G1MixedGCCountTarget, G1OldCSetRegionThresholdPercent);
G1FullCollector::G1FullCollector(G1CollectedHeap* heap,
                                 bool explicit_gc,
                                 bool clear_soft_refs,
                                 bool do_maximum_compaction) :
//    adaptive_mixedgc(G1MixedGCLiveThresholdPercent, G1HeapWastePercent),
    _heap(heap),
