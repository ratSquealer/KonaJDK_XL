...
​	下面虽然是这个方法的整体思路，但是和官方的Adaptive IHOP的思路还是很不一样的：**我们没有用到历史信息**，关于这个，可以通过`G1 源码+IHOP 源码`可以获取一些底层的历史信息，**包括但不限于：老区的晋升速度、垃圾处理速度、老区已使用大小、大对象分配速度等**。

​	很明显这些历史信息，更能揭发底层的一些逻辑，从而可能更好的表达G1 heap的当前情况，从而更好的调整这几个关键参数。

​	**但是！**在观察`C_set.log`文件和`IHOP.log`文件时：

- 期望条件：我们期望1次mixed gc阶段（可能含多次）的垃圾处理大小能和1次G1 GC整个阶段（startTime_n+1_mark -startTime_n_mark) （后面简称one_circle_time）垃圾产生大小达到一个平稳可控的范围
- 观察情况：在`C_set.log`文件和`IHOP.log`文件中，IHOP体现的老区分配速度（相对均衡） *  one_circle_time < Cset体现的垃圾处理大小（HWP * heapSize）。

​	**因此，虽然也可以说，单这两个历史数据不够的话，应该引入其他类型的历史数据（例如年轻代扩展大小等）。但是这两个数据时比较典型的跟老区高度相关的，因此观察到这个情况的时候，综合来看，就没有对其纳入本次的规则算法指定中。**

​	**观察图片：![1956fccc-7940-4bad-a4d9-009c37f0b9b9](imgs/1956fccc-7940-4bad-a4d9-009c37f0b9b9.png)**
...
**G1MixedGCLiveThresholdPercent 和 HeapWastePercent：**

G1MixedGCLiveThresholdPercent 下降，导致所需处理的垃圾增加，HWP会相应下降就会处理更多的垃圾。

因为HWP的上升，导致IHOP的动态下降，以期望更好的触发全局Mark阶段，减少To-Space的可能性

当HWP下降时，垃圾容忍度下降，导致更少的Candidate Set中的region 会被剪枝，因此会处理更多的regions数量

![image-20241012204618249](imgs/image-20241012204618249.png)

![image-20241012204930463](imgs/image-20241012204930463.png)

**G1OldCSetRegionThresholdPercent and G1MixedGCCountTarget**：

相同的，当Candidate Set 数量增加同时（HWP不变或者下降），就会导致 Collection Set的数量增加，在时间允许的条件下，应该增加Collection Set的**左右区间**，调整如下：

- 当Pruned Candidate Set数量达到阈值后，G1MixedGCCountTarget 下降 —> Cset 的最小值上升（Min 2 4regions-->4regions）
- 当Pruned Candidate Set数量达到阈值后，G1OldCSetRegionThresholdPercent 上升—> Cset的最大值上升（Max 7regions --> 8regions）

![76beb274-ebaa-445a-a47a-81ceb402f5ae](imgs/76beb274-ebaa-445a-a47a-81ceb402f5ae.png)
